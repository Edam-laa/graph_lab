...
class WeightDialog(QDialog):
    def __init__(self, parent=None, default_weight=1, default_capacity=1, is_directed=False, weighted=True):
...
class EdgeItem(QGraphicsPathItem):
    ...
    def update_path(self):
        ...
        # Show "w/c" if capacity meaningful, else just weight
...
class GraphScene(QGraphicsScene):
    ...
    def get_properties(self) -> dict:
        ...
        # Complete check
        complete = all(len(adj[i]) >= n-1 for i in ids)
...
class MainWindow(QMainWindow):
    ...
    def _build_sidebar(self):
        ...
        for btn in [self._btn_select, self._btn_add_node, self._btn_add_edge, self._btn_delete]:
    ...
    def _run_algo(self, name):
        ...
        # Sync selector dropdown to the button clicked
        for i, (n, _, _) in enumerate(ALGO_DEFS):
            if n == name:
                self._algo_selector.setCurrentIndex(i)
                break

        algo_name, algo_cat = self._get_selected_algo_info()
        source = self._param_source.text().strip()
        sink   = self._param_sink.text().strip()
        params = {}
        if source: params["source"] = source
        if sink:   params["sink"]   = sink

        data  = self._scene.get_graph_data(algo_name=algo_name, algo_category=algo_cat, algo_params=params)
        ...
        complexity_map = {
            ...
            "Kruskal":              "O(E log E)",
            "Prim":                 "O((V + E) log V)",
            "Composantes Connexes": "O(V + E)",
            "Chemin Eulérien":      "O(E)",
            "Welsh-Powell":         "O(V² + E)",
        }
        complexity = complexity_map.get(name, "O(?)")

        # ── Update Execution Info labels ──────────────────────────────────────
        t0 = time.perf_counter()

        self._exec_labels["exec_status"].setText("running…")
        self._exec_labels["exec_status"].setStyleSheet(f"color: {ACCENT_AMBER}; font-weight: bold;")
        self._exec_labels["exec_complexity"].setText(complexity)
        self._exec_labels["exec_complexity"].setStyleSheet(f"color: {ACCENT_CYAN}; font-weight: bold;")
        self._exec_labels["exec_time"].setText("…")

        # ── Write to log ──────────────────────────────────────────────────────
        self._log_separator()
        self._log(f"{name}  [{complexity}]", "header")
        self._log(f"Nodes: {len(nodes)}   Edges: {len(edges)}   "
                  f"Directed: {data['graph']['directed']}   "
                  f"Weighted: {data['graph']['metadata']['weighted']}", "info")
        if params:
            self._log(f"Params: {params}", "info")
        self._log_separator()

        # ── Algorithm-specific steps ──────────────────────────────────────────
        if name in ("Bellman-Ford", "Bellman"):
            self._log_bellman_ford(nodes, edges, source, sink, data["graph"]["directed"])

        elif name == "Dijkstra":
            self._log_dijkstra(nodes, edges, source, sink)

        elif name == "Kruskal":
            self._log_kruskal(nodes, edges)

        elif name == "Prim":
            self._log_prim(nodes, edges, source)

        elif name == "Composantes Connexes":
            self._log_components(nodes, edges, data["graph"]["directed"], props)

        elif name == "Chemin Eulérien":
            self._log_euler(nodes, edges, props, data["graph"]["directed"])

        elif name == "Welsh-Powell":
            self._log_welsh_powell(nodes, edges)

        # ── Timing ───────────────────────────────────────────────────────────
        t1 = time.perf_counter()
        exec_ms = round((t1 - t0) * 1000, 3)
        self._log_separator()
        self._log(f"Execution time (frontend): {exec_ms} ms", "info")
        self._log("JSON exported to algorithm.params — send to backend.", "dim")

        # Update exec labels
        self._exec_labels["exec_status"].setText("ready")
        self._exec_labels["exec_status"].setStyleSheet(f"color: {ACCENT_CYAN}; font-weight: bold;")
        self._exec_labels["exec_time"].setText(f"{exec_ms} ms")
        self._exec_labels["exec_time"].setStyleSheet(f"color: {TEXT_MAIN};")

        # Show badge
        self._log_algo_badge.setText(f"  {name}  ")
        self._log_algo_badge.show()
    ...
    def _log_bellman_ford(self, nodes, edges, source, sink, directed):
        if not source:
            self._log("⚠  No source node specified. Set 'Source node' in the right panel.", "warn")
            return
        if source not in nodes:
            self._log(f"✖  Source node '{source}' not found in graph.", "error")
            return
        ...
        self._log(f"Initialise distances from source '{source}':", "step")
        self._log(f"dist = {{ {source}: 0, others: ∞ }}", "dim")
        self._log("", "dim")
        ...
        for iteration in range(len(nodes) - 1):
            ...
            for e in edges:
                ...
                if dist[u] != INF and dist[u] + w < dist[v]:
                    dist[v] = dist[u] + w
                    pred[v] = u
                    changes.append(f"dist[{v}] = {dist[v]}  (via {u})")
                    updated = True
                if not directed:
                    if dist[v] != INF and dist[v] + w < dist[u]:
                        dist[u] = dist[v] + w
                        pred[u] = v
                        changes.append(f"dist[{u}] = {dist[u]}  (via {v})")
                        updated = True
            if changes:
                self._log(f"Iteration {iteration + 1}:", "step")
                for c in changes:
                    self._log(c, "dim")
            if not updated:
                self._log(f"No updates in iteration {iteration + 1} — early stop.", "info")
                break
        ...
        self._log("", "dim")
        self._log("Final distances:", "step")
        for n in nodes:
            d = dist[n] if dist[n] != INF else "∞"
            self._log(f"  {source} → {n}  =  {d}", "result" if dist[n] != INF else "dim")

        if neg_cycle:
            self._log("⚠  Negative cycle detected! Distances may be incorrect.", "warn")

        if sink and sink in nodes:
            path = []
            cur = sink
            while cur is not None:
                path.insert(0, cur)
                cur = pred[cur]
            if dist[sink] != INF:
                self._log(f"Shortest path {source}→{sink}: {' → '.join(path)}  (cost: {dist[sink]})", "result")
            else:
                self._log(f"No path from {source} to {sink}.", "warn")

    def _log_dijkstra(self, nodes, edges, source, sink):
        if not source:
            self._log("⚠  No source node specified. Set 'Source node' in the right panel.", "warn")
            return
        if source not in nodes:
            self._log(f"✖  Source '{source}' not found.", "error")
            return
        ...
        pred = {n: None for n in nodes}
        visited = set()
        adj = self._node_map(edges)

        self._log(f"Initialise: dist[{source}] = 0, all others = ∞", "step")
        self._log("Priority queue (greedy — always pick min dist node):", "info")
        ...
        while True:
            ...
            if u is None or dist[u] == INF:
                break
            visited.add(u)
            self._log(f"Step {step}: Visit '{u}'  (dist={dist[u]})", "step")
            step += 1
            ...
            for (v, w, _) in adj.get(u, []):
                if v not in visited:
                    ...
                    if nd < dist[v]:
                        dist[v] = nd
                        pred[v] = u
                        self._log(f"  Relax edge {u}→{v}  weight={w}  new dist[{v}]={nd}", "dim")

        self._log("", "dim")
        self._log("Final shortest distances:", "step")
        for n in nodes:
            d = dist[n] if dist[n] != INF else "∞"
            self._log(f"  {source} → {n}  =  {d}", "result" if dist[n] != INF else "dim")

        if sink and sink in nodes and dist[sink] != INF:
            path, cur = [], sink
            while cur:
                path.insert(0, cur); cur = pred[cur]
            self._log(f"Optimal path {source}→{sink}: {' → '.join(path)}  (cost: {dist[sink]})", "result")
        elif sink:
            self._log(f"No path from {source} to {sink}.", "warn")

    def _log_kruskal(self, nodes, edges):
        self._log("Sort all edges by weight (ascending):", "step")
        sorted_edges = sorted(edges, key=lambda e: e.get("weight", 1))
        for e in sorted_edges:
            self._log(f"  {e['from']} — {e['to']}  weight={e.get('weight',1)}", "dim")
        ...
        self._log("", "dim")
        self._log("Build MST — add edge if it doesn't create a cycle:", "step")
        mst, total = [], 0
        for e in sorted_edges:
            u, v, w = e["from"], e["to"], e.get("weight", 1)
            if union(u, v):
                mst.append(e)
                total += w
                self._log(f"  ✔ Add  {u} — {v}  (w={w})  MST size: {len(mst)}", "result")
            else:
                self._log(f"  ✖ Skip {u} — {v}  (would form cycle)", "info")
            if len(mst) == len(nodes) - 1:
                break

        self._log("", "dim")
        self._log(f"MST total weight: {total}   Edges: {len(mst)}", "result")
        if len(mst) < len(nodes) - 1:
            self._log("⚠  Graph is not connected — MST incomplete.", "warn")

    def _log_prim(self, nodes, edges, source):
        start = source if source and source in nodes else (nodes[0] if nodes else None)
        if not start:
            self._log("✖  Graph has no nodes.", "error"); return

        adj = self._node_map(edges)
        in_mst = {start}
        mst_edges, total = [], 0

        self._log(f"Start from node '{start}'", "step")
        ...
        while len(in_mst) < len(nodes):
            ...
            total += w
            self._log(f"  Add edge {u} — {v}  (w={w})   in MST: {sorted(in_mst)}", "result")

        self._log("", "dim")
        self._log(f"MST total weight: {total}   Edges: {len(mst_edges)}", "result")
        if len(in_mst) < len(nodes):
            self._log("⚠  Graph is not connected — some nodes unreachable.", "warn")

    def _log_components(self, nodes, edges, directed, props):
        adj = {n: [] for n in nodes}
        adj_rev = {n: [] for n in nodes}
        for e in edges:
            adj[e["from"]].append(e["to"])
            adj_rev[e["to"]].append(e["from"])
            if not directed:
                adj[e["to"]].append(e["from"])

        # BFS connected components (undirected view)
        visited, comp_id, comp_num = {}, {}, 0
        self._log("BFS to find connected components:", "step")
        for start in nodes:
            if start not in visited:
                ...
                while q:
                    cur = q.pop(0)
                    grp.append(cur)
                    comp_id[cur] = comp_num
                    for nb in adj[cur]:
                        if nb not in visited:
                            visited[nb] = True; q.append(nb)
                self._log(f"  Component {comp_num}: {{ {', '.join(sorted(grp))} }}", "result")

        self._log("", "dim")
        self._log(f"Total components: {comp_num}", "result")
        self._log(f"Connected: {'Yes' if comp_num == 1 else 'No'}", "result" if comp_num == 1 else "warn")

        if directed:
            self._log("", "dim")
            self._log("Checking strongly connected components (Kosaraju sketch):", "step")
            self._log("  (Full SCC requires two DFS passes — send JSON to backend for result)", "info")

    def _log_euler(self, nodes, edges, props, directed):
        has_circuit = props.get("eulerian_circuit", False)
        has_path    = props.get("eulerian_path", False)

        self._log("Check Eulerian conditions:", "step")
        if directed:
            self._log("  For directed graph:", "info")
            self._log("  • Eulerian circuit  → every vertex: in-degree = out-degree", "dim")
            self._log("  • Eulerian path     → exactly one vertex with out−in=1 (start),", "dim")
            self._log("                        exactly one with in−out=1 (end)", "dim")
        else:
            self._log("  For undirected graph:", "info")
            self._log("  • Eulerian circuit  → all vertices have even degree", "dim")
            self._log("  • Eulerian path     → exactly 2 vertices have odd degree", "dim")
        ...
        self._log("", "dim")
        self._log("Degree table:", "step")
        for n in nodes:
            if directed:
                diff = deg_out[n] - deg_in[n]
                mark = "✔" if diff == 0 else ("⚑ start" if diff == 1 else ("⚑ end" if diff == -1 else "✖"))
                self._log(f"  {n}: out={deg_out[n]}  in={deg_in[n]}  diff={diff:+d}  {mark}", "dim")
            else:
                mark = "✔ even" if deg[n] % 2 == 0 else "⚑ odd"
                self._log(f"  {n}: deg={deg[n]}  {mark}", "dim")

        self._log("", "dim")
        if has_circuit:
            self._log("✔  Graph HAS an Eulerian Circuit (visits every edge exactly once, returns to start)", "result")
        elif has_path:
            self._log("✔  Graph HAS an Eulerian Path (visits every edge exactly once)", "result")
            if directed:
                start_nodes = [n for n in nodes if deg_out[n] - deg_in[n] == 1]
                end_nodes   = [n for n in nodes if deg_in[n] - deg_out[n] == 1]
                if start_nodes: self._log(f"  Start node: {start_nodes[0]}", "result")
                if end_nodes:   self._log(f"  End node:   {end_nodes[0]}", "result")
            else:
                odd_nodes = [n for n in nodes if deg[n] % 2 != 0]
                if len(odd_nodes) == 2:
                    self._log(f"  Start/End nodes (odd degree): {odd_nodes[0]}  and  {odd_nodes[1]}", "result")
        else:
            self._log("✖  Graph has NO Eulerian path or circuit.", "warn")
            self._log("   Add or remove edges so all degree conditions are satisfied.", "dim")

    def _log_welsh_powell(self, nodes, edges):
        ...
        self._log("Step 1 — Sort nodes by degree (descending):", "step")
        self._log("  " + "  ".join(f"{n}({degrees[n]})" for n in sorted_nodes), "dim")

        color_map = {}
        color_num = 0
        self._log("", "dim")
        self._log("Step 2 — Greedy coloring:", "step")

        for node in sorted_nodes:
            if node in color_map:
                continue
            color_num += 1
            color_map[node] = color_num
            self._log(f"  Color {color_num} → '{node}'", "result")
            # Extend color to non-adjacent uncolored nodes
            for other in sorted_nodes:
                if other not in color_map and other not in adj[node]:
                    # Check other is not adjacent to any already colored with color_num
                    if not any(color_map.get(nb) == color_num for nb in adj[other]):
                        color_map[other] = color_num
                        self._log(f"  Color {color_num} → '{other}'  (not adjacent to color {color_num} group)", "dim")

        self._log("", "dim")
        self._log(f"Chromatic number χ(G) ≤ {color_num}", "result")
        self._log("Final coloring:", "step")
        by_color = {}
        for n, c in color_map.items():
            by_color.setdefault(c, []).append(n)
        for c in sorted(by_color):
            self._log(f"  Color {c}: {{ {', '.join(sorted(by_color[c]))} }}", "result")
