...
class WeightDialog(QDialog):
    def __init__(self, parent=None, default=1, is_directed=False):
        ...
        self.setStyleSheet(f"""
            ...
            QPushButton[flat=true] {{ background: {BG_CARD2}; color: {TEXT_DIM}; }}
        """)
        ...
        btn_box.setStyleSheet(f"""
            QPushButton {{ background: {ACCENT_BLUE}; color: white; border-radius: 5px;
                           padding: 7px 18px; }}
            QPushButton[text="Cancel"] {{ background: {BG_CARD2}; color: {TEXT_DIM}; }}
        """)
...
class EdgeItem(QGraphicsPathItem):
    ...
    def update_path(self):
        ...
        self._weight_item.setPlainText(w_txt)
...
class GraphScene(QGraphicsScene):
    ...
    def _finalize_edge(self, dst: NodeItem):
        ...
        # Ask weight if weighted
        weight = 1.0
        if self._weighted_graph:
            parent_widget = self.views()[0].window() if self.views() else None
            dlg = WeightDialog(parent=parent_widget, is_directed=self._directed_graph)
            if dlg.exec():   # non-zero == Accepted in PySide6
                weight = dlg.get_weight()
            else:
                return

        edge = EdgeItem(src, dst, weight, self._directed_graph)
    ...
    def get_graph_data(self) -> dict:
        nodes = [{"id": n.node_id, "label": n.label,
                  "x": n.scenePos().x(), "y": n.scenePos().y()}
                 for n in self._nodes.values()]
        edges = [{"source": e.src.node_id, "target": e.dst.node_id,
...
class MainWindow(QMainWindow):
    ...
    def _apply_global_style(self):
        self.setStyleSheet(f"""
            ...
            #bottomBar {{ background: {BG_PANEL}; border-top: 1px solid {BORDER}; }}
    ...
    def _on_graph_changed(self):
        ...
        # Update algo buttons
        self._update_algo_buttons(props, data)
    ...
    def _update_algo_buttons(self, props, data):
        n = props.get("nodes", 0)
        m = props.get("edges", 0)
    ...
    def _run_algo(self, name):
        data = self._scene.get_graph_data()
        msg = f"Running <b>{name}</b> on graph with "
        msg += f"{len(data['nodes'])} nodes and {len(data['edges'])} edges.<br><br>"
        msg += "The graph JSON is ready to send to your backend."
        dlg = QMessageBox(self)
        dlg.setWindowTitle(f"Algorithm: {name}")
        dlg.setTextFormat(Qt.RichText)
        dlg.setText(msg)
        dlg.setIcon(QMessageBox.Information)
        # Show the JSON preview
        preview = json.dumps(data, indent=2)[:600] + ("..." if len(str(data)) > 600 else "")
        dlg.setDetailedText(json.dumps(data, indent=2))
        dlg.exec()
