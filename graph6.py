    ...
    def apply_result(self, data):
        ...
        for e in self.scene._edges:
            if (e.src.label, e.dst.label) in mst_set or (e.dst.label, e.src.label) in mst_set:
                e.setPen(QPen(QColor(ACCENT_GREEN), 4))
...
class WeightDialog(QDialog):
    def __init__(self, parent=None, default_weight=1, default_capacity=1, is_directed=False, weighted=True, capacity_enabled=True):
        ...
        self.setStyleSheet(f"""
            ...
            QLineEdit {{ background: {BG_CARD2}; color: {TEXT_MAIN}; border: 1px solid {BORDER};
...
class NodeItem(QGraphicsEllipseItem):
    ...
    def _update_appearance(self):
        ...
        if self.isSelected():
            ...
            pen_c = QColor(ACCENT_BLUE_ACTIVE)
        else:
    ...
    def hoverEnterEvent(self, event):
        grad = QRadialGradient(0, -5, self.RADIUS*1.2)
        grad.setColorAt(0, QColor("#DBEAFE"))
        grad.setColorAt(1, QColor(NODE_HOVER))
        self.setBrush(QBrush(grad))
        self.setPen(QPen(QColor(ACCENT_BLUE), 2))
...
class EdgeItem(QGraphicsPathItem):
    def __init__(self, src: NodeItem, dst: NodeItem, weight=1.0, capacity=1, directed=True):
        ...
        self._pen = QPen(QColor(EDGE_COLOR), 2.0, Qt.SolidLine, Qt.RoundCap)
            ...
            if self._mode == "add_node":
                ...
                if node:
                    if self._edge_src is None:
                        ...
                        self._temp_line.setPen(QPen(QColor(ACCENT_BLUE), 1.5, Qt.DashLine))
...
class GraphView(QGraphicsView):
    ...
    def drawBackground(self, painter, rect):
        ...
        if not self._draw_grid:
            return
        painter.setPen(QPen(QColor(GRID_COLOR), 0.8))
        step = 40
...
class ToolBtn(QPushButton):
    ...
    def _update_style(self):
        if self.isChecked():
            self.setStyleSheet(f"""
                QPushButton {{ background: {self.active_color}18;
                               color: {self.active_color}; border: 1px solid {self.active_color}88;
            ...
            self.setStyleSheet(f"""
                ...
                QPushButton:hover {{ background: {BG_CARD2}; color: {TEXT_LABEL}; border: 1px solid {BORDER}; }}
            """)
...
class AlgoBtn(QPushButton):
    ...
    def set_available(self, avail: bool):
        ...
        if avail:
            ...
            self.setStyleSheet(f"""
                QPushButton {{ background: {self._color}15; color: {self._color};
                               border: 1px solid {self._color}55; border-radius: 6px; }}
                QPushButton:hover {{ background: {self._color}28; border: 1px solid {self._color}; }}
                QPushButton:pressed {{ background: {self._color}40; }}
            """)
        else:
            self.setEnabled(False)
            self.setStyleSheet(f"""
                QPushButton {{ background: {BG_CARD2}; color: {TEXT_DISABLED};
...
def section_label(text):
    ...
    lbl.setStyleSheet(f"color: {TEXT_DISABLED}; letter-spacing: 1px; padding: 4px 0;")
    return lbl
...
class MainWindow(QMainWindow):
    ...
    def _build_sidebar(self):
        ...
        self._btn_delete   = ToolBtn("Delete",        "✕", ACCENT_RED)
        ...
        self._style_toggle(self._chk_capacity, ACCENT_CYAN)
        ...
        self._btn_clear = self._action_btn("⌫  Clear Graph", ACCENT_RED)
        ...
        self._btn_undo = self._action_btn("↶ Undo", ACCENT_PURPLE)
        self._btn_undo.clicked.connect(self._scene.undo)
        layout.addWidget(self._btn_undo)

        self._btn_show_result = self._action_btn("🧠 Show Result JSON", ACCENT_PURPLE)
    ...
    def _build_bottom_bar(self):
        ...
        algo_lbl = QLabel("Algorithms:")
        algo_lbl.setStyleSheet(f"color: {TEXT_DIM}; font-family: Consolas; font-size: 11px;")
    ...
    def _build_log_panel(self):
        ...
        self._log_algo_badge.setStyleSheet(
            f"color: {ACCENT_BLUE}; background: {ACCENT_BLUE}18; "
            f"border: 1px solid {ACCENT_BLUE}55; border-radius: 3px; padding: 1px 6px;"
        )
        ...
        self._log_text.setStyleSheet(f"""
            ...
            QScrollBar:vertical {{
                background: {BG_CARD2}; width: 8px; border: none;
            }}
            ...
            QScrollBar:horizontal {{
                background: {BG_CARD2}; height: 8px; border: none;
            }}
    ...
    def _build_right_panel(self):
        ...
        self._algo_selector.setStyleSheet(f"""
            QComboBox {{ background: {BG_CARD2}; color: {TEXT_MAIN}; border: 1px solid {BORDER};
                         border-radius: 4px; padding: 4px 8px; }}
            QComboBox::drop-down {{ border: none; }}
            QComboBox QAbstractItemView {{ background: {BG_PANEL}; color: {TEXT_MAIN};
                                           selection-background-color: {ACCENT_BLUE}28; border: 1px solid {BORDER}; }}
        """)
        ...
        self._matrix_table.setStyleSheet(f"""
            QTableWidget {{ background: {BG_PANEL}; color: {TEXT_MAIN};
                            border: 1px solid {BORDER}; gridline-color: {DIVIDER}; font-size: 10px; }}
            ...
            QTableWidget::item:selected {{ background: {ACCENT_BLUE}22; }}
        """)
    ...
    def _field_style(self):
        return (f"QLineEdit {{ background: {BG_CARD2}; color: {TEXT_MAIN}; "
    ...
    def _style_toggle(self, btn, color):
        ...
        def _upd(checked):
            if checked:
                btn.setStyleSheet(f"""QPushButton {{ background: {color}18; color: {color};
                ...
                btn.setStyleSheet(f"""QPushButton {{ background: transparent; color: {TEXT_DIM};
                    ...
                    QPushButton:hover {{ background: {BG_CARD2}; }}""")
    ...
    def _action_btn(self, text, color):
        ...
        btn.setCursor(Qt.PointingHandCursor)
        btn.setStyleSheet(f"""
            QPushButton {{ background: {color}15; color: {color}; border: 1px solid {color}55;
                           border-radius: 6px; text-align: left; padding: 0 12px; }}
            QPushButton:hover {{ background: {color}28; border: 1px solid {color}; }}
        """)
        return btn
    ...
    def _apply_global_style(self):
        self.setStyleSheet(f"""
            ...
            #logPanel {{ background: {BG_PANEL}; border-top: 1px solid {BORDER}; }}
            #logHeader {{ background: {BG_CARD2}; border-bottom: 1px solid {BORDER}; }}
        ...
        for i in range(n):
            for j in range(n):
                ...
                if val is None:
                    ...
                    color = QColor(ACCENT_BLUE) if i != j else QColor(ACCENT_AMBER)
    ...
    def _update_properties(self, props):
        ...
        bool_map = {True: ("Yes", ACCENT_GREEN), False: ("No", ACCENT_RED)}

        def set_bool(key, val):
            txt, col = bool_map.get(val, ("—", TEXT_DISABLED))
        ...
        if props.get("regular"):
            ...
            self._info_labels["regular"].setStyleSheet(f"color: {ACCENT_GREEN}; font-weight: bold;")
        else:
            self._info_labels["regular"].setText("No")
            self._info_labels["regular"].setStyleSheet(f"color: {ACCENT_RED}; font-weight: bold;")

        ec = props.get("eulerian_circuit", False)
        ep = props.get("eulerian_path", False)
        ec_txt, ec_col = bool_map.get(ec, ("—", TEXT_DISABLED))
        ep_txt, ep_col = bool_map.get(ep, ("—", TEXT_DISABLED))
    ...
    def _reset_graph_style(self):
        for e in self._scene._edges:
            e.setPen(QPen(QColor(EDGE_COLOR), 2.0))
    ...
    def _highlight_path(self, path):
        ...
        for e in self._scene._edges:
            if (e.src.label, e.dst.label) in pairs:
                e.setPen(QPen(QColor(ACCENT_RED), 4))
...
def main():
    ...
    palette.setColor(QPalette.Dark,            QColor(BG_CARD2))
    palette.setColor(QPalette.Light,           QColor(BG_PANEL))
    palette.setColor(QPalette.Midlight,        QColor(BG_DARK))
    palette.setColor(QPalette.Shadow,          QColor(BORDER))
